...
redes_opt = ['LinkedIn', 'Twitch', 'YouTube', 'Instagram', 'Amigos', 'Twitter / X', 'Outra rede social']
...
with col1:
    video_game = st.radio("Curte Vídeo Game?", ["Sim", "Não"])
    futebol = st.radio("Curte Futebol?", ["Sim", "Não"])
    
    idade = st.number_input("Sua Idade", 18, 100)
    
    tempo_opt = ['De 0 a 6 meses', 'De 1 ano a 2 anos', 'De 6 meses a 1 anos', 'Não atuo', 'De 2 ano a 4 anos', 'Mais de 4 anos']
    tempos = st.selectbox("Tempo que atua na área de dados", options=tempo_opt)
...
with col2:
    ...
    uf = st.selectbox("Estado que mora atualmente", options=uf_opt)
    
    nivel_opt = ['Iniciante', 'Júnior', 'Pleno', 'Sênior', 'Especialista', 'Coordenação', 'Gerência', 'Diretoria', 'C-Level']
    cadeira = st.selectbox("Posição da cadeira (senioridade)", options=nivel_opt)

with col3:
    f1 = st.radio("Curte Fórmula 1?", ["Sim", "Não"])
    mma = st.radio("Curte MMA?", ["Sim", "Não"])
    areas_opt = ['Biológicas','Exatas','Humanas']
    formacao = st.selectbox("Área de Formação", options=areas_opt)
...
if proba > 0.7:
    st.success(f"Você é uma pessoa feliz! 😄 Probabilidade de ser feliz: {100 * proba:.0f}%")
elif proba > 0.4:
    st.warning(f"Você está na média! 🤔 Probabilidade de ser feliz: {100 * proba:.0f}%")
elif proba <= 0.4:
    st.error(f"Você não é uma pessoa feliz! 😞 Probabilidade de ser feliz: {100 * proba:.0f}%")
